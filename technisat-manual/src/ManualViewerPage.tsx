...
function ManualViewerPage() {
  ...
  useEffect(() => {
    ...
    fetchManualData(parseInt(manualId, 10)); // Parse ID from URL string

  }, [manualId]); // Re-fetch if manualId changes
  ...
  useEffect(() => {
    ...
    if (currentTab.content_type === 'steps') {
       ...
       audioId = `${currentTab.tab_key}_main`;
        ...
        if (listContent && listContent.length > 0) {
            // Play audio for the *first* item when tab is selected
            // To play for specific items, interaction would be needed
            if (currentSubStepIndex === 0) { // Only play when tab first selected
                 audioId = listContent[0].id;
            }
        }
    }
    ...
    if (audioPath && audioRef.current.src !== `${window.location.origin}${audioPath}`) {
  ...
  }, [activeTabIndex, currentSubStepIndex, manualData, isLoading, error]);
  ...
  if (error) return <div className="error-message">Error loading manual {manualId}: {error}</div>;
  if (!manualData) return <div className="error-message">No data found for manual {manualId}.</div>;

  return (
    // Use the same App class for overall styling
    <div className="App">
      <header>
